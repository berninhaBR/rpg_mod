...
package net.mcreator.rpgcraft.keybind;
...
import net.mcreator.rpgcraft.procedures.DasherOnKeyPressedProcedure;
...
public class DasherKeyBinding extends RpgCraftModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public DasherKeyBinding(RpgCraftModElements instance) {
		super(instance, 2);
	...
	public void initElements() {
		keys = new KeyBinding("key.mcreator.dasher", GLFW.GLFW_KEY_END, "key.categories.movement");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					RpgCraftMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
